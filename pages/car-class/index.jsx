...
const CarClass = ({ allAirportData }) => {
  ...
  const { relevantSchemas, loadingSchemas } = useFindRelevantSchemas(
    range,
    airport_id,
    date
  );
                  ...
                  <>
                    ...
                    <div class="car-card" key={id}>
                      ...
                      <div class="detail-wrap">
                        ...
                        {/* Car Description */}
                        <p class="font-bold text-gray-dark">{description}</p>
                        {/* Car More Details */}
                            ...
                            <div class="flex max-sm:flex-col justify-between items-center">
                              ...
                              <button
                                onClick={() => {
                                  ...
                                  const booklink = `/car-details?booking_type=${booking_type}&airport_id=${airport_id}&hotel_place_id=${hotel_place_id}&car_class_id=${item.id}&date=${date}&guest_number=${guest_number}&selected_price=refundable`;
                                  if (session) {
                          ...
                          {relevant_non_refundable_price > 0 && (
                            <div class="flex max-sm:flex-col justify-between items-center">
                              ...
                              <button
                                onClick={() => {
                                  // Generate booklink to details after select car
                                  const booklink = `/car-details?booking_type=${booking_type}&airport_id=${airport_id}&hotel_place_id=${hotel_place_id}&car_class_id=${item.id}&date=${date}&guest_number=${guest_number}&selected_price=nonRefundable`;
                                  if (session) {
                                    window.location.href = booklink;
                                    return;
                                  }
